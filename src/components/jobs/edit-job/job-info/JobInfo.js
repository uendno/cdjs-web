...
class JobInfoComponent extends Component {
  ...
  _renderRepoTypeButton(type) {
    ...
    return (
      <Button
        className={`button-with-icon repo-type-button ${type === repoType ? 'selected' : null}`}
        onClick={() => {
      ...
      <FormGroup>
        ...
        <ButtonToolbar>
          <DropdownButton title={this._getCurrentCredentialName()} id="dropdown-size-medium">
            <MenuItem
              eventKey="1"
              onClick={() => updateBeingEditedJob({credential: null})}
            >None
            </MenuItem>
    ...
    return (
      <FormGroup>
        ...
        <p className="note">Copy this url then add it to Webhooks section in your repository
                    settings to enable automatic job execution.
        </p>
      </FormGroup>
    );
  }
