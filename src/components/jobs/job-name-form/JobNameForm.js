...
const throttledCheckJobName = _.throttle((name, id, updateEditJobFormData, updateBeingEditedJob, checkJobName) => {
  checkJobName(name, id).then((result) => {
    if (result.valid === false) {
      updateEditJobFormData({loading: false, invalidNameMessage: 'A job with this name already exists'});
    } else {
      updateEditJobFormData({loading: false});
    }
  });
}, 1000, {
  leading: false,
...
class JobNameFormComponent extends Component {
  _handleNameInputChange(e) {
    ...
    updateBeingEditedJob({name});

    updateEditJobFormData({invalidNameMessage: '', loading: true});
    ...
    return (
      <div className="job-name-form-component">
        <FormGroup
          ...
          <p className="note">Please note that job name must be unique in order to
...
const mapStateToProps = state => ({data: getDataForJobNameComponent(state)});

export default connect(mapStateToProps, {checkJobName, updateBeingEditedJob, updateEditJobFormData: updateEditJobModalData})(JobNameFormComponent);